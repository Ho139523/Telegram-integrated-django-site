...
from accounts.models import ProfileModel, Address, TelStatus
...
class SendLocation:
        ...
        try:
            ...
            for code, country in items:
                if country in buttons:
                    ...
                    handlers[f'country_{code}'] = lambda c: self.handle_picked_country(c)

            buttons['🔙'] = {'callback_data': 'address_back', 'index': len(buttons)+1}
            handlers["address_back"] = lambda c: self.handle_previous(c)

            buttons['❌ بستن'] = {'callback_data': 'address_close', 'index': len(buttons)+2}
            handlers["address_close"] = lambda c: self.handle_close(c)
            layout.append(2)


            data = {"state": "address_selection_country"}
            self.session_manager.set_user_session(self.chat_id, data, namespace="address")
            ...
            print(self.session_manager.get_user_session(self.user_id, namespace="address"))
    ...
    def handle_prev(self, call):
        try:
            session = self.session_manager.get_user_session(self.user_id, namespace="address")
            #print(self.session_manager.get_user_session(self.user_id, namespace="address"))
            ...
            if session['state'] == "address_selection_country":
                ...
                for code, city in items:
                    if city in buttons:
                        ...
                        handlers[f'city_{code}'] = lambda c: self.handle_picked_city(c)
            ...
            #data = self.session_manager.get_user_session(self.user_id, namespace="address")

        except Exception as e:
            print(f"Error in handle_prev: {e}")
            self.app.send_message(self.chat_id, "خطایی در پیمایش رخ داد")
    ...
    def handle_next(self, call):
        try:
            ...
            print(f"Error in handle_next: {e}")
            self.app.send_message(self.chat_id, "خطایی در پیمایش رخ داد")
    ...
    def handle_picked_country(self, call):
        ...
        #print(self.session_manager.get_user_session(self.user_id, namespace="address"))
        self.session_manager.get_user_session(self.user_id, namespace="address")["state"] = "address_selection_province"
        self.session_manager.get_user_session(self.user_id, namespace="address")["selected_country"] = f"{country}"
        self.session_manager.set_user_session(self.chat_id, self.session_manager.get_user_session(self.user_id, namespace="address"), namespace="address")
    ...
    def handle_picked_city(self, call):
        ...
        markup.edit(call.message.message_id) 
        status, created = TelStatus.objects.get_or_create(profile=self.profile)
        status.status = "address_selection_street"
        status.save()
        data = self.session_manager.get_user_session(self.user_id, namespace="address")
        print(data)
    ...
    def handle_picked_street(self, message):
        ...
        self.app.send_message(self.chat_id, text)


        TelStatus.objects.get_or_create(profile=self.profile, status="address_selection_zipcode")
